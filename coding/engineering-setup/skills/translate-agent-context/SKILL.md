...
description: >
  ...
  `review-agent-md` and `review-claude-md`. Does NOT discover or classify rules
...
This skill is the only home for cross-runtime mechanism translation, orchestrator decomposition, and shared `skills/` layout. It does not rewrite content shape — that's `review-claude-md` (CLAUDE.md) and `review-agent-md` (AGENTS.md). It does not discover rules from code — that's `extract-rules`.

## Scope: What This Skill Owns

| In scope (this skill) | Out of scope — handoff |
|---|---|
| Translate `.claude/rules/*.md` → AGENTS.md inline / `docs/conventions/*.md` | AGENTS.md structure audit → `review-agent-md` |
| Translate slash commands → shell scripts in `tools/` | CLAUDE.md content audit → `review-claude-md` |
...
Do not run this skill just to clean up AGENTS.md prose — use `review-agent-md`. Do not run it to discover what rules the codebase already enforces — use `extract-rules`.
...
- No opportunistic folder cleanup — that belongs to `review-agent-md` (AGENTS.md) or `review-claude-md` (CLAUDE.md).
...
After translation produces or modifies AGENTS.md, recommend `review-agent-md` to clean up structure (canonical 9-section skeleton, Agent Surface table, 220-line ceiling). This skill makes mechanism-level changes, not structural ones.
...
If translation reveals that CLAUDE.md violates the canonical 9-section structure or exceeds 200 lines, recommend `review-claude-md`. This skill does not rewrite CLAUDE.md content shape.
