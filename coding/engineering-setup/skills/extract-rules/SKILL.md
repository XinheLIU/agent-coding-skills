...
description: >
  ...
  CLAUDE.md or AGENTS.md prose — see `review-claude-md` and `review-agent-md`.
...
This skill does not own the project's *what/why* sections (Project Overview, architecture, modules, data tables, test coverage). Those belong to `review-claude-md` (CLAUDE.md) and `review-agent-md` (AGENTS.md).

## Scope: What This Skill Owns

| In scope (this skill) | Out of scope — handoff |
|---|---|
| Static rules (naming, formatting, API patterns, DB conventions, security, logging) | Project Overview / business scenarios → `review-claude-md` |
| Dynamic per-task guardrails (scope limits, confirmation policies) | Architecture decisions → `review-claude-md` |
| Implicit conventions ("Do not delete this section", "Used by Partner A") | Module/directory layout → `review-claude-md` |
...
| `docs/conventions/*.md` (long rules in AGENTS.md-only repos) | AGENTS.md structural rewrite → `review-agent-md` |
| `docs/spec.md` authoring | CLAUDE.md structural rewrite → `review-claude-md` |
| Adding rows to CLAUDE.md / AGENTS.md `Context Files` table | Table shape/format of `Context Files` table → `review-claude-md` / `review-agent-md` |
...
**Do not restructure CLAUDE.md or AGENTS.md sections.** If CLAUDE.md violates the canonical 9-section structure or is over 200 lines, recommend `review-claude-md`. If AGENTS.md is over 220 lines or fails MECE, recommend `review-agent-md`.
...
If the scan reveals that CLAUDE.md violates the canonical 9-section structure, exceeds 200 lines, has duplicated content, or contains rule-shaped prose that should be evicted, **do not refactor it here**. Recommend the user run `review-claude-md` for structural rewrite. This skill only adds rows to the `Context Files` table.

## Handoff to review-agent-md

If the scan reveals that AGENTS.md exceeds 220 lines, has duplicated content across `Coding Standards` / `Special Constraints` / referenced docs, or fails MECE in its referenced doc set, recommend `review-agent-md` for structural rewrite. This skill adds rules and `Context Files` rows to AGENTS.md but does not restructure it.
