...
- **Greenfield mode**: the user has a PRD, idea, desired capability, or incomplete workflow.
...
If verification fails because a fact is missing, ask for that fact. If it fails because the design is weak, revise the design before presenting it.
