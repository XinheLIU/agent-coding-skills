---
name: acs-review-implementation-gaps
...
You are comparing the actual state of the codebase to what a design document promised. Your output is a structured gap analysis artifact that (a) tells the user what to build next and (b) feeds into `acs-review-code-quality` as the next pipeline stage.
...
- As Stage 2 of the review pipeline: plan-review → **gap-review** → acs-review-code-quality
...
Use exactly this structure — `acs-review-code-quality` parses it:
...
Reviewer: acs-review-implementation-gaps
...
Focus acs-review-code-quality on: <comma-separated list of component names with status COMPLETE or PARTIAL>
```
...
**Suppress findings with confidence ≤ 4** from the main output — the gap analysis is about status, not code quality. Defer quality review to `acs-review-code-quality`.

## What this skill does NOT do

- Does not review code quality in depth (DRY, edge cases, perf) — that's `acs-review-code-quality`.
- Does not challenge the plan itself — that's `acs-review-design-doc`.
...
Recommended next skill: /acs-review-code-quality (will consume this artifact and review the COMPLETE+PARTIAL components)
```
