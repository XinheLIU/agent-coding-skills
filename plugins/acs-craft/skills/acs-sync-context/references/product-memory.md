...
The [shared protocol](../../acs-init-context/references/PROTOCOL.md) owns the two memory classes, relationships, freshness and handoffs. In Persistent `product.html`, mission/vision/principles are Intent; evidenced capabilities are Current; tickets, proposed or accepted scope and decisions are Changes. Exploratory `discovery.html` is Working until needed for formal review, cross-run handoff or downstream reliance; save those conclusions and evidence as proposed Persistent records before that boundary. Classify records independently of format or acceptance.
...
Save proposals required for review or downstream reliance with `proposed` status and the minimum evidence; saving them does not accept scope or bypass product ownership. A contributing skill without authority to amend durable product intent saves an addressable proposal in the canonical change home, linking the product subject and consumed revision; the coordinator routes intent reconciliation to `acs-write-prd`. It does not edit accepted product intent merely to make its proposal Persistent. Promote settled product intent with the minimum evidence and rationale needed to understand it. Preserve stable IDs when moving records. When amending an existing durable subject, reconcile the accepted amendment into that record rather than creating a duplicate. Link supporting records once; an overview or requirements index links to them rather than repeating their normative text. Keep raw code inventories in Working Memory; maintain useful observed product summaries in Persistent Current with source revision and coverage. Historical deltas and rationale belong to Persistent Changes; point to code for executable facts.
...
Human review and derived companions follow [Presenter](../../../resources/protocols/presenter.md), including version-scoped decisions, deduplication and retrievable reviewed content. The rules below preserve the canonical HTML reading format rather than creating a separate fact source.
