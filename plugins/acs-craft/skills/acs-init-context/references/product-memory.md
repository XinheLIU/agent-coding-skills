...
The [shared protocol](PROTOCOL.md) owns the two memory classes, relationships, freshness and handoffs. In Persistent `product.html`, mission/vision/principles are Intent; evidenced capabilities are Current; tickets, proposed or accepted scope and decisions are Changes. Exploratory `discovery.html` is Working until needed for formal review, cross-run handoff or downstream reliance; save those conclusions and evidence as proposed Persistent records before that boundary. Classify records independently of format or acceptance.
