...
For the compact card grammar, standalone packaging rule, and before/after visual budget, also read [the local visual report contract](../../../authoring/references/visual-report.md). This document remains the product-memory-specific presentation guidance; the local contract supplies the portable visual rules.

Read [the product memory contract](../../../../../skills/acs-init-context/references/product-memory.md) first. The baseline enriches `discovery.html`; it does not own the document or produce a separate `current-product.html`. These patterns guide presentation of current behavior inside shared concepts. Preserve records, IDs, and styling already present.
