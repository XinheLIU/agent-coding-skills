...
Use `<article id="gap-export-feedback" data-kind="gap">` for an addressable record within a semantic `<section>`, per the [HTML record format](html-records.md). The `data-kind` vocabulary is a **closed list**:
...
1. Resolve the product and effort. Read the document `<nav>`, then fetch only the target records and their evidence, decisions, and dependencies using the [retrieval patterns](html-records.md#targeted-retrieval). Do not load every record merely because they share a file.
...
7. Verify before reporting:
   - Every record satisfies the [HTML record format](html-records.md): unique `<kind>-<slug>` ID, closed-list `data-kind` inside a shared section, complete `<nav>` index, resolving links and promotion pointers, current `Last updated` dates.
...
Serialization of shared writes follows the [HTML record format](html-records.md#writing) and the shared protocol's coordinated-writes rules.
...
Product documents follow the [HTML record format](html-records.md) for structure, self-containment, retrieval, and write discipline. Human review and derived companions follow [Presenter](presenter.md), including version-scoped decisions, deduplication and retrievable reviewed content. Product-specific additions: `data-kind`, `data-ticket-type`, `data-commitment`, and `data-depth` are part of the reading contract, and product mockups are separate, explicitly illustrative prototype artifacts, not a second memory store.
