...
The active agent performs this coordinator role under the [shared protocol](../protocols/skill-declarations.md), including when invoking a skill standalone. This is a written Harness contract, not a new executable service. Domain skills return results and proposed transitions; the coordinator applies shared state changes through capabilities actually available in the session.

The [harness architecture](../resources/docs/harness-architecture.md) defines source identity, standalone packaging, and the boundary with Codex, Claude Code, Pi, OpenCode, and DeepSeek harness adapters. Keep domain procedures and shared state semantics here; put host APIs, invocation syntax, worker executables, and lifecycle integrations in their binding. A plugin manifest alone does not establish runtime support.
...
When human reading benefits from a view, invoke [Presenter](../protocols/presenter.md) as a responsibility of the active agent: pass canonical references/revisions and the reading or decision focus. Domain owners provide meaning; Presenter provides layout. A view is optional in a handoff and cannot be its only evidence.
