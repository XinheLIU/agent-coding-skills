...
The user receives one `docs/changes/<change-id>/delivery-plan.html` after accepted scope has been decomposed. Preserve an established report location on resumed work. `acs-plan-delivery` owns domain scope, decomposition, blockers, and recommendations; [Presenter](../../../../../protocols/presenter.md) owns display and review semantics. `acs-draw-portfolio-dag` renders the derived view and can refresh progress without repeating planning.
