...
Presenter preserves substantive meaning, recommendation order, uncertainty, blocking effects and domain status. Missing reasoning returns to its domain owner. It cannot accept a proposal, change ticket status or infer authorization. Simple results use text/Markdown; use HTML when diagrams, comparisons or progress benefit from it. Follow the shared [visual report format](../../skills/acs-audit-architecture/references/visual-report.md) for a derived HTML companion. No separate Presenter skill is required for domain work or text delivery.
