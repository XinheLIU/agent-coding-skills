...
At a gate, the design skill returns accepted decision/contract and surface references, consumed revisions, delta, questions, and next action. The coordinator applies run-state updates, capability discovery/dispatch, and serialized writes under [workflow coordination](protocol:acs:context-coordination). Capability rows are Run Context; disjoint rows do not permit concurrent file writes.
