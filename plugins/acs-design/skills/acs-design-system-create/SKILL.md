...
Create the canonical design authority that grounds all design work in this project: `DESIGN.md` at the project root — the **how** of the shared design triad ([references/design-memory.md](../../resources/protocols/design-memory.md)).
...
Do NOT use when:
- `DESIGN.md` already exists and is current (validate and use it; `/acs-design-context` re-syncs it)
- Only exploring variants without defining the system (use `/acs-visual-design-variants`)
- Implementing an already-approved design (use `/acs-design-implement`)
...
**Downstream:**
- `DESIGN.md` at project root → feeds `acs-design-interaction-flow`, `acs-visual-design-variants`, and `acs-design-implement`
...
if [ -f DESIGN.md ]; then
  ...
  echo "LEGACY_SYSTEM_MD: yes — route to /acs-design-context to migrate"
else
  echo "EXISTING_DESIGN_AUTHORITY: no"
fi
```

A legacy `docs/design/system.md` with no `DESIGN.md` is a migration, not a creation — hand off to `/acs-design-context` Step 1.
...
**Shortcut worth offering:** if a published spec already fits this product type closely, adopting it wholesale via `/acs-design-context` beats composing one from scratch. Offer it; proceed only on approval.
...
- Tokens created from scratch via acs-design-system-create, YYYY-MM-DD
- Product context: configured product document persona/problem records
- Component specs: docs/design/components/
```

If `docs/design/prototype.html` exists, re-sync its `:root` token block from the new frontmatter (see `/acs-design-context` Step 4).
...
- Use `/acs-design-implement` to convert approved designs into production code
...
**Feeds:**
- `acs-visual-design-variants` (uses DESIGN.md as constraint)
- `acs-design-implement` (uses DESIGN.md for tokens/patterns)
- `spec` (references design authority for component specs)

**When a reference exists instead:** `/acs-design-context` imports it rather than proposing from scratch. This skill is the no-reference path.
