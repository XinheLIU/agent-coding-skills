...
- You want to design user flows or screens — that's `/acs-design-interaction-flow`, which runs after this
- Only generating visual options on an existing structure — that's `/acs-visual-design-variants`
...
**Downstream:**
- `DESIGN.md` at project root (Current State, git-tracked) — the canonical **how**; feeds `/acs-design-interaction-flow`, `/acs-visual-design-variants`, `/acs-design-implement`, `build/acs-implement`, and the `:root` token block of `docs/design/prototype.html`
...
Request optional ④ templates or ⑤ acs-design-context results through the coordinator. Preserve DESIGN.md authority and reconcile provenance/tokens before acceptance. Native adoption/extraction/creation remains available.
...
>
> **A)** Run `/acs-design-system-create` — consultative from-scratch creation, native to this repo (Recommended)
...
- **Design authority source:** [existing DESIGN.md / migrated from system.md / extracted from <url> / adopted from <catalog> / created via /acs-design-system-create]
...
>
> **A)** Run `/acs-design-interaction-flow` — define structure and states in the canonical prototype (Recommended for new features)
> **B)** Run `/acs-visual-design-variants` — structure already locked, go straight to visuals
...
**Feeds:**
- `/acs-design-interaction-flow` (reads DESIGN.md constraints)
- `/acs-visual-design-variants` (applies DESIGN.md tokens)
- `/acs-design-implement` (production code uses DESIGN.md tokens)
- `spec` (design constraints cited as input)

**When no import tool is available:** the from-scratch path (`/acs-design-system-create`) is the fallback and nothing is blocked.
