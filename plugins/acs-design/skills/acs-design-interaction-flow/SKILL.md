---
name: acs-design-interaction-flow
description: "Decide how a feature behaves before deciding how it looks — information architecture, user flows, and the five interaction states (empty, loading, ideal, error, partial), locked as wireframe sections of the canonical prototype (docs/design/prototype.html). Requires product context; produces the structure that /acs-visual-design-variants is forbidden to change. Use for UX flow, wireframe, state design, or interaction design work on a feature whose behavior is unsettled."
---
...
- Before visual design exploration (feeds into `/acs-visual-design-variants`)
...
**Downstream:**
- `docs/design/prototype.html` — wireframe-fidelity sections, one per surface, all five states rendered, `data-structure="locked"` at approval (Change Context; the hard prerequisite of `/acs-visual-design-variants`)
...
The locked wireframe sections in the prototype are what `/acs-visual-design-variants` styles; `state-table.md` remains the analysis `/acs-design-implement` reads for state semantics.
...
**Read PRD Part 3** if exists. If complete, use it as foundation. If missing/incomplete, record the proposed interaction states and link the affected capability/question IDs for reconciliation via `acs-write-prd`.
...
If A: hand the accepted state-table changes and affected capability IDs to `acs-write-prd` for minimal reconciliation into canonical records. Preserve unrelated requirements; do not create a second normative five-state copy.
...
> **C)** Need to see visual mockups — skip to /acs-visual-design-variants
...
- Create the file from scratch when absent: self-contained HTML, no build step, one shared keyboard-accessible state switcher driving the `data-state` blocks, and a `:root` block — grayscale wireframe values now, `DESIGN.md` tokens synced in later by `/acs-design-context` or applied by `/acs-visual-design-variants`.
...
**PRD Integration:**
- [✅/❌] Accepted state definitions handed to acs-write-prd

**Ready for Next Step:**
Structure is locked in the canonical prototype. `/acs-visual-design-variants` will
...
> **Next step:**
>
> **A)** Run `/acs-visual-design-variants` now (explore visual directions based on this structure)\
...
**Feeds into:**
- `/acs-visual-design-variants` — styles the locked wireframe sections in the canonical prototype
- `/acs-design-implement` — uses state table to generate all states in code
...
**Writes to (optional):**
- Product capability/acceptance records — via `acs-write-prd` when sync is authorized
