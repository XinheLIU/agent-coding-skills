...
Design one experiment to answer one question. Do not define the product, scope multiple assumptions at once, or minimize the product as a side effect. The product's target experience lives in `acs-shape-solution`; commitments live in `acs-define-outcomes`.
...
| --- | --- | --- |
| **Persevere** | The assumption held and users demonstrated the behavior | Return to `acs-define-outcomes`; the evidence may raise a `deferred` capability or lift a `reduced` depth |
...
- `acs-shape-solution` if the evidence changes the target experience
- `acs-define-outcomes` if the solution holds but a commitment level or depth needs adjustment
...
- **Does not define the product** — the solution shape stays in `acs-shape-solution`
- **Does not commit scope** — commitments belong in `acs-define-outcomes`
- **Does not replace demand validation** — for general demand uncertainty, use `acs-validate-demand`
