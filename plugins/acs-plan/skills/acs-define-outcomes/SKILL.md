...
For existing products, require source-backed evidence of the affected current behavior before defining a delta. Reuse supplied or shared baseline evidence; route missing behavior inspection to `acs-map-current-product`.

When improvement value is disputed, preserve the unresolved question and route to `acs-validate-demand` before treating the change as validated.
...
- Excluded and deferred items are explicit enough for `acs-write-prd` and `spec` to preserve.

## What This Skill Does NOT Do

- **Does not validate demand** — it defines outcomes for a solution already judged worth building
- **Does not shape the solution** — `acs-shape-solution` produces the target experience; this skill commits it
- **Does not locate the solution in scope space** — `acs-shape-solution` resolves scenario, product form, and data availability before capabilities can be judged
...
- **Does not write the PRD** — it produces outcome contracts; `acs-write-prd` consolidates them
- **Does not run experiments** — use `acs-design-experiment` to resolve specific uncertainties cheaply
