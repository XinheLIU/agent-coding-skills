...
2. Read canonical ticket status and any active run's next action. Follow the chosen skill's `context.requires` and relevant `retrieves` selectors. Product HTML: read navigation, then only the relevant articles and their premises; do not load every domain contract.
