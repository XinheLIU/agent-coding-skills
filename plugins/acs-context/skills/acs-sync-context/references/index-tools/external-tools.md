# External Indexers for the Code Index Layer

Last updated: 2026-08-25

The Code Index layer is machine-generated: it answers WHAT and HOW so no document has to. These four tools build it. They solve the same problem — give the agent a queryable map of the code instead of repeated greps — and trade speed against depth. Pick one; running several produces redundant indexes that drift apart.

This file is the evidence behind the choice. It is optional in every repository: enable an index only when querying one beats reading the code directly.
...
None of the above build a curated prose knowledge base, and none should be asked to. A code index is derived from code and is rebuildable at any time; distilled reading notes are authored, carry judgment, and belong to whatever knowledge-base tooling the user already has. Do not route prose into the code index, and do not let an indexer's output stand in for a decision record.
...
- **Record the choice in the routing file.** Whichever file agents read first must say which indexer this repo uses, where the index lives, how to query it, and how to refresh it. An index the agent does not know about is dead weight.
