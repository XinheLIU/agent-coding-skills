...
description: >
  ...
  query smells, missing timeouts, perf hotspots, test gaps) belong to `acs-review-code-quality`.
---
...
Read [the visual report contract](../acs-refactor-code/references/visual-report.md) and [the HTML report format](references/HTML-REPORT.md) before generating an HTML companion. These local references are bundled with the skill; do not depend on `/meta` or another installed skill.
...
## Boundary with `acs-review-code-quality` (MECE)
...
Rule of thumb: *acs-review-architecture judges the system's design; acs-review-code-quality judges the code that implements it.* When a subagent surfaces something on the wrong side of the line, route it via the cross-reference channel — do not silently flag it as a finding here.
...
Note: architecture-level findings often need surrounding context that a narrow diff cannot show. Prefer **Whole codebase** or **Subtree** unless the user specifically wants change-scoped review. For change-scoped questions tied to a PR, `acs-review-code-quality` (Mode A) is usually the better fit.
...
- **Cross-reference recommendations** to `acs-review-code-quality` go in their own section, not as findings here.
...
- **Code-level follow-ups (route to `acs-review-code-quality`):** <list with file paths and reason>
...
When an HTML companion is requested or generated, use the local visual report contract: compact scorecard, one card per major finding or recommendation, current/target diagrams for structural changes, an explicit comparison-not-applicable reason for score-only findings, collapsed evidence, and one anchored top recommendation. The Markdown report remains canonical.
...
- Route code-level findings to `acs-review-code-quality` via the cross-reference channel.

**DON'T:**
- Promote LOW-confidence findings to Critical.
- Paraphrase reviewer findings.
- Skip the Executive Summary or the ADR Ledger.
- Flag code-level defects (handler bugs, perf smells, query injection) here — those are `acs-review-code-quality`'s job.
