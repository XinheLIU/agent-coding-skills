...
ACS has one set of domain skills and shared context contracts. Its delivery targets are a complete host plugin/extension and individually installable skills with the same public IDs. This milestone establishes naming and architectural boundaries; it does not implement a scheduler, host SDK integrations, or standalone package isolation.
...
Canonical status stays in the configured tracker. Run state and execution handles are disposable projections. A later restart must reread current tickets, claims, revisions, and evidence before resuming; it cannot replay a stale write or restore authorization from an adapter's private state.
...
Current source packages still link outside their own directories, and the context subpackage is a materialized development view with similar dependencies. Neither copying a directory nor the existing context builder proves standalone usability.

Acceptance for a future standalone build: move the generated package into an isolated directory with no source checkout or sibling skills available, resolve and execute its required resources, and complete representative tasks. Start with `acs-research`, `acs-tdd`, `acs-review-code-quality`, and `acs-implement`; then cover every public skill. Check authorized serial operation, optional-skill absence, unavailable required tools, and equivalent evidence in plugin and standalone modes.
