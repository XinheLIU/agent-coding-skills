...
def validate(suite: Path = SUITE) -> list[str]:
    ...
    if len(skills) != 49:
        errors.append(f"expected 49 source skills, found {len(skills)}")
    for path in skills:
        text = path.read_text(encoding="utf-8")
...
if __name__ == "__main__":
    ...
    for finding in findings:
        print(finding)
    print(f"49 source skills, discovery symlinks, declarations, local links/anchors: {'FAIL' if findings else 'PASS'}")
