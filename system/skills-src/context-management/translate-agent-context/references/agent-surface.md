...
Translation requires splitting the role into (a) a runtime-neutral prompt in `agents/<role>.md` and (b) a small per-runtime binding that points at it plus the runtime's invocation mechanism. See `runtime-parity-translation.md` for the concrete pattern.
