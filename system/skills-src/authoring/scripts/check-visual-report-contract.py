...
CONSUMERS = (
    ...
    ROOT / "quality/review/acs-review-architecture",
    ROOT / "quality/review/acs-refactor-code",
)


def main() -> int:
    errors: list[str] = []
    canonical = ROOT / "craft/meta/references/visual-report.md"
    if not canonical.exists():
        errors.append(f"missing canonical contract: {canonical}")
    for skill_dir in CONSUMERS:
        for relative in ("SKILL.md", "references/visual-report.md", "references/HTML-REPORT.md"):
