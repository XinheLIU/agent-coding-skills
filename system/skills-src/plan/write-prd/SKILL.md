...
3. Assign the ticket type:
   ...
   - **Prototype ticket** (`data-ticket-type="prototype"`): a design question blocks specification. The ticket carries a one-or-two-sentence prototype request as the handoff to `design/ux/validate-prototype` and stays `blocked` until the decision lands; then convert it to a spec ticket linking the decision, or close it.
