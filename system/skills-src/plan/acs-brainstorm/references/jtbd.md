...
The framework behind the JTBD brief that `acs-brainstorm` produces. Read this when the Specify phase needs the full pillar detail, the interview probes, or the output template.
...
Not this stage. Deciding which capabilities are committed, reduced, deferred, or excluded belongs to `acs-define-outcomes`, after `acs-validate-demand` has issued a verdict and `acs-shape-solution` has made the journey concrete.
...
These constrain form without choosing it. The form decision itself belongs to `acs-shape-solution`.
...
Record it and stop. `acs-validate-demand` will grade this as Inferred-level evidence and the verdict will reflect it.
...
The Assumptions table is what `acs-validate-demand` grades. Every claim in the brief that came from the team rather than from a user belongs in it.
