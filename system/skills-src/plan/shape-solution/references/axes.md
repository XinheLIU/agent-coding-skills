...
Resolve the axes **in `shape-solution`, before `define-outcomes` sets commitment and depth.** A
...
Write the resolution as one sentence before handing the shape to `define-outcomes`:
