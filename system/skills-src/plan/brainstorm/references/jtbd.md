...
Not this stage. Deciding which capabilities are committed, reduced, deferred, or excluded belongs to `define-outcomes`, after `validate-demand` has issued a verdict and `shape-solution` has made the journey concrete.
...
These constrain form without choosing it. The form decision itself belongs to `shape-solution`.
