...
```text
Greenfield:
1. Is the demand real?      →  acs-brainstorm → acs-validate-demand
2. What is the solution?    →  acs-shape-solution          (key artifact: target experience)
3. What are the commitments?→  acs-define-outcomes         (verifiable promises + deferred intent)

Existing product:
1. What exists today?       →  acs-map-current-product      (key artifact: current behavior)
2. Is the improvement real? →  acs-validate-demand          (active-product evidence)
3. What changes and proves done? →  acs-define-outcomes     (ADDED / MODIFIED / REMOVED + verification)

Optional: acs-design-experiment — resolve a named uncertainty cheaply before committing
```
...
If a Green demand assessment or accepted scope exists only in working memory, route to `acs-write-prd` for promotion before continuing. If routing and an unambiguous destination are absent, recommend `acs-init-context` before persistence; existing evidence can still support an in-conversation diagnosis.
...
| Situation | Route to |
| --- | --- |
| "What does this codebase/app do?" or "write user stories from this codebase" | `acs-map-current-product` |
| "Improve/iterate/refine this existing app" and affected current behavior is not evidenced | `acs-map-current-product` (next: `acs-define-outcomes`) |
...
| Idea exists, the struggle isn't articulated | `acs-brainstorm` |
| Struggle named, demand unproven or disputed | `acs-validate-demand` |
| Demand just graded Green, nothing in the tracked layer yet | `acs-write-prd` (early mode), then `acs-shape-solution` |
| Demand verdict Green, no solution shape | `acs-shape-solution` |
| Solution shape exists, outcomes not committed | `acs-define-outcomes` |
| Specific assumption needs cheap validation before committing | `acs-design-experiment` |
| Current product mapped, increment undefined | `acs-define-outcomes` |
| Outcomes defined, risks unexamined | `acs-run-premortem` |
| Accepted conclusions are unpromoted, or the PRD needs reconciliation after an increment | `acs-write-prd` to extend it |
| Demand evidence is assumption-only | back to `acs-validate-demand` |
| A scope change was accepted in premortem | `acs-define-outcomes` to update outcome contracts |
| Effort shipped but its PRD was never written | `acs-write-prd`, or `acs-sync-context` for the wider sweep |
...
- `acs-research` — trace a claim to primary sources when evidence is disputed
- `acs-engineer-domain-model` — resolve vocabulary when terms are used inconsistently
- `acs-explore-unknowns` — use instead of this router when the route spans more than one session
