...
**The Problem:**
Diagrams with 15+ elements become unreadable in Mermaid. The C4 model allows 20, but Mermaid's Dagre layout engine degrades noticeably past 15.
...
### 4. Too Many Relationships (Edge Density)

**The Problem:**
Edge count matters more than node count for layout quality. A diagram with 12 nodes and 18 edges produces as many crossing arrows as one with 20 nodes — Mermaid cannot route dense edge sets cleanly.

**Rule of thumb:** If `|edges| > |elements| × 1.5`, the diagram will likely have overlapping labels and crossing arrows.

**Wrong — 8 nodes, 14 edges (1.75 ratio):**
```mermaid
C4Container
  title WRONG: Too many cross-cutting relationships

  Container(a, "Service A", "Java", "Core service")
  Container(b, "Service B", "Go", "Processing")
  Container(c, "Service C", "Python", "Analytics")
  ContainerDb(db1, "DB A", "PostgreSQL", "A's data")
  ContainerDb(db2, "DB B", "PostgreSQL", "B's data")
  ContainerDb(db3, "DB C", "PostgreSQL", "C's data")
  Container(gateway, "Gateway", "Nginx", "Ingress")
  System_Ext(ext, "External API", "Third-party")

  Rel(gateway, a, "Routes", "HTTP")
  Rel(gateway, b, "Routes", "HTTP")
  Rel(gateway, c, "Routes", "HTTP")
  Rel(a, db1, "Reads/writes", "SQL")
  Rel(b, db2, "Reads/writes", "SQL")
  Rel(c, db3, "Reads/writes", "SQL")
  Rel(a, b, "Delegates", "gRPC")
  Rel(b, c, "Feeds", "Kafka")
  Rel(a, ext, "Fetches", "REST")
  Rel(b, ext, "Fetches", "REST")
  Rel(c, a, "Reports back", "gRPC")
  Rel(c, b, "Queries", "gRPC")
  Rel(a, c, "Triggers", "Kafka")
  Rel(b, a, "Notifies", "Kafka")
```

**Correct — split into focused diagrams:**
```mermaid
C4Container
  title CORRECT: Service A — direct dependencies only

  Container(gateway, "Gateway", "Nginx", "Ingress")
  Container(a, "Service A", "Java", "Core service")
  ContainerDb(db1, "DB A", "PostgreSQL", "A's data")
  Container(b, "Service B", "Go", "Downstream processor")
  System_Ext(ext, "External API", "Third-party")
...
### 5. Long Relationship Labels Causing Overlap

**The Problem:**
Mermaid renders relationship labels along the edge line and does not wrap them. Labels longer than ~40 characters collide with adjacent labels and node text when edges cross.

**Wrong — label too long:**
```
Rel(nginx, data_service, "Proxies /data/ prefix; transforms read-only SQL queries", "HTTP / port 7080")
```

**Correct — verb + protocol only, ≤40 chars; detail lives in the element description:**
```
Rel(nginx, data_service, "Proxies /data/ prefix", "HTTP")
```

If a relationship genuinely needs more context, add it to the target container's description field, not the arrow label. Example:
```
Container(data_service, "data_service", "FastAPI, FastMCP", "Read-only HTTP + MCP gateway; enforces schema allowlist and READ ONLY transactions")
```
...
- [ ] All relationship labels ≤40 characters (verb + protocol only)
- [ ] Diagram has a clear, specific title
- [ ] ≤15 elements (Mermaid layout degrades past 15)
- [ ] Edge count ≤ elements × 1.5 (split if over — edge density causes overlap)
- [ ] `UpdateLayoutConfig` present when ≥5 elements
