...
**Required for any diagram with ≥5 elements.** Omitting it on larger diagrams leaves Mermaid using a 4-per-row default that produces wide layouts with many crossing edges.

**Recommended starting points by element count:**

| Elements | `$c4ShapeInRow` | `$c4BoundaryInRow` | Notes |
|---|---|---|---|
| 5–8 | `"3"` | `"1"` | Compact, avoids wrapping |
| 9–12 | `"2"` | `"1"` | Forces two-column layout, reduces horizontal crossings |
| 13–15 | `"2"` | `"1"` | Maximum safe range for Mermaid; prefer splitting at this count |

Place `UpdateLayoutConfig` as the **last statement** in the diagram (after all elements and relations), so it applies to the fully-defined graph.

**Example — reducing crowding in a Container diagram:**
```mermaid
C4Container
  ...
  Rel(api, extApi, "Fetches data", "REST")
...
**Complex Diagrams:**
Keep diagrams under 15 elements (the C4 model allows 20, but Mermaid's layout engine degrades before that). Split complex architectures into multiple focused diagrams. Also split when edge count exceeds elements × 1.5, regardless of node count — edge density is the more common cause of layout collapse.
