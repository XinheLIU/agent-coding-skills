...
- Working: `<work-root>/<effort>/discovery.html` — findings, alternatives, assessments, unresolved analysis, and research completeness tracking for one effort.
- Human: `<product-docs>/<product-slug>/product.html` — durable product understanding, accepted intent, the roadmap, and the PRD reading view.
- Specs: `<product-docs>/<product-slug>/specs/<spec-slug>.md` — standalone feature specifications generated by `write-prd`, linked from roadmap tickets. Other skills may draft a loose spec; recommend tightening it through `write-prd` before engineering consumes it.
...
MECE guides the boundary, not a hard constraint: `discovery.html` is disposable working memory; `product.html` is the hard handoff. A fact needed after the effort ends must reach `product.html`; analysis in progress stays in discovery.
...
- **Spec ticket** (`data-ticket-type="spec"`): requirements are settled enough to specify. Links a standalone Markdown spec in `specs/<spec-slug>.md`, generated by `write-prd`. A loose spec drafted by another skill may be linked provisionally; the ticket notes that `write-prd` should tighten it.
...
Every ticket records: priority (`p0` / `p1` / `p2` / `deferred`), status (`proposed` / `accepted` / `in-progress` / `blocked` / `completed` / `abandoned`), its research basis (links to the gap/opportunity/problem records that justify it), and its alignment (links to at least one mission or vision).
...
Ticket lifecycle: `proposed → accepted → in-progress → completed`, with `blocked` for prototype tickets awaiting a decision and `deferred` / `abandoned` as exits. When a prototype decision is captured (`<work-root>/<effort>/prototypes/<slug>/decision.md`), the ticket links the decision and either converts to a spec ticket (via `write-prd`) or closes. A ticket does not close a behavior gap; only evidence of the resulting behavior does.

Roadmap tickets are product intent, not engineering task state. `engineering/feature/spec` consumes a ticket's spec file; task decomposition retains its own contract.
