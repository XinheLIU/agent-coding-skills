...
description: Review or update a repository's AGENTS.md or CLAUDE.md. Use to encode one incident lesson, repair instruction hierarchy and pointers, prune ineffective guidance, or reconnect shared-context routing; use translate-agent-context for runtime migration.
---
...
When the signal is ambiguous and the difference changes edit scope, ask whether the user wants the single lesson recorded or the whole file reviewed. If both runtime files exist and the problem is parity rather than content quality, hand off to `translate-agent-context`.
...
Point to configured values instead of copying them. When memory routing is absent, hand off to `init-context`; when it exists but its targets are stale, hand off to `sync-context`.
