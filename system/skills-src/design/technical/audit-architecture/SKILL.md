...
description: Assess existing codebase architecture — module boundaries, coupling, cohesion, tech debt. Entry point for brownfield refactoring. Use when code is messy, tangled, hard to change, or needs structural improvement.
---
...
context:
  ...
  handoff_to: [refactor_boundaries, split_module, design]
```
...
- Proposed fix direction (defer details to `refactor-boundaries`)
...
- Proposed split or merge (defer details to `split-module`)
...
**Recommended fix:** [direction only — defer to refactor-boundaries]
...
**Priority:** High/Medium/Low
**Recommended fix:** [direction only — defer to split-module]
...
1. **[Fix type]** — [module/area] — Priority: High — Feeds: [refactor-boundaries/split-module]
2. [...]
```
...
Route the selected work to:
- `refactor-boundaries` — for boundary violations
- `split-module` — for cohesion problems
- `engineer-domain-model` — if naming is the core issue
- `harden-architecture` — for greenfield redesign
...
This skill surveys; it does not fix. Implementation belongs in `refactor-boundaries`, `split-module`, or `build/incremental`.

If the codebase is greenfield (no existing structure to assess), use `harden-architecture` instead.

If specific functionality is unclear, defer to `engineer-domain-model` for domain modeling.

## Cross-references

- Downstream: `refactor-boundaries` (fix boundary violations)
- Downstream: `split-module` (decompose god objects)
- Downstream: `engineer-domain-model` (resolve naming issues)
- Related: `harden-architecture` (greenfield architecture design)
