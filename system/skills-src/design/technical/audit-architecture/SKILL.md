...
description: Reconstruct current module boundaries, dependencies, coupling, and compatibility constraints before brownfield design. Use when the existing structure or runtime path cannot be established cheaply; use review-architecture for holistic architecture judgment.
---
...
- Proposed fix direction (defer details to `design-modules`)
...
- Proposed split or merge (defer details to `design-modules`)
...
**Recommended fix:** [direction only — defer to design-modules]
...
1. **[Fix type]** — [module/area] — Priority: High — Feeds: [design-architecture/design-modules]
2. [...]
```
...
Route the selected work to `design-architecture` when the target system shape is unsettled, or to `design-modules` when the target is selected and needs boundary, cohesion, interface, or migration contracts. Use `engineer-domain-model` when shared terminology is the blocking issue.
...
This skill surveys; it does not fix. Design changes belong in `design-architecture` or `design-modules`; implementation belongs in `build/implement`.

If the codebase is greenfield (no existing structure to assess), use `design-architecture` instead.

If specific functionality is unclear, defer to `engineer-domain-model` for domain modeling.

## Cross-references

- Downstream: `design-architecture` (select the target system shape)
- Downstream: `design-modules` (design boundaries, splits, interfaces, and migration)
- Downstream: `engineer-domain-model` (resolve naming issues)
- Related: `challenge-approach` (stress-test proposed changes)
