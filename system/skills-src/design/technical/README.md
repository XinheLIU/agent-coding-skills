...
`explore-unknowns` is an optional wayfinding utility for a decision that spans sessions or has independent prerequisites. NFR analysis, domain clarification, module splitting, boundary repair, and adversarial challenge are techniques inside the owning node, not required standalone phases.
