...
2. Identify shared capabilities by real consumers: communication, persistence, identity, errors, observability, scheduling, storage, UI primitives, or domain policies. Keep shared domain rules with their owning domain.
...
5. Produce `technical-design.md` (or the established design home) containing feature-to-module mapping, capability consumers, option comparison, selected architecture, gaps, and open decisions. Include a readable diagram and stable IDs for modules and capabilities.
...
Send shared capability rows to `design-foundation` and business/module rows to `design-modules`. A greenfield foundation may be designed before all feature modules when it is needed to validate the first real path. A brownfield audit is required only when current structure or compatibility is unknown; use `audit-architecture` for that evidence.
