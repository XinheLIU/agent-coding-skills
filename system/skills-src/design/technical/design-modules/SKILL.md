...
For each module define one responsibility, its public interface (including invariants and failures), owned types, dependencies, callers, registration/wiring location, and verification surface. Apply the deletion test and prefer a deep interface with local implementation complexity. Add seams for real variation; reuse a framework directly when it already provides the required contract.

Cover routes, menus, commands, jobs, exports, adapters, middleware order, and dependency injection where they are part of the feature path. For brownfield work, preserve observable behavior with characterization evidence, introduce facades or adapters where useful, and sequence caller migration with rollback points.
...
Update `technical-design.md` with a module table, interface contracts, dependency direction, route/menu/export wiring, shared capability usage, tests, and migration steps. The result is implementation-ready when a builder can locate every symbol, registration point, and acceptance criterion without inventing ownership.
