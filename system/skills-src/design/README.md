...
- **Enter `ux/`** when what the user sees or navigates is undecided. Shared design understanding is a triad — `product.html` is why, root `DESIGN.md` is how, `docs/design/prototype.html` is what. The UX pipeline runs in stages — enter at the earliest unsettled one:
  ...
  - `ux/interaction-design` — flows, layout, information hierarchy, or five-state coverage undecided; locks wireframe sections in the canonical prototype
...
Every design skill can read the product document resolved through `docs/agents/memory.md` and `state.md` (`product.html#prd`, or a canonical legacy PRD). Follow relevant record anchors and open questions; each skill also runs standalone.
